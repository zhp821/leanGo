...
func syncApps(jsonapps *MarathonApps) bool {
	...
	for _, app := range jsonapps.Apps {
		...
		if app.Labels["HAPROXY_GROUP"] != config.Realm && app.Labels["NGINX_GROUP"] != config.Realm {
			continue
		}
		...
		for _, task := range app.Tasks {
			...
			newtask.ID = task.ID
			fmt.Println(newtask.Ports)
			newapp.Tasks = append(newapp.Tasks, newtask)
		}
		// Lets ignore apps if no tasks are available
		if len(newapp.Tasks) > 0 {
			var nginxHost = ""
			if app.Labels["HAPROXY_0_VHOST"] != "" {
				nginxHost = app.Labels["HAPROXY_0_VHOST"]
			} else if app.Labels["NGINX_VHOST"] != "" {
				nginxHost = app.Labels["NGINX_VHOST"]
			}
			if nginxHost != "" {
				hosts := strings.Split(nginxHost, ",")
				...
				for _, host := range hosts {
					...
					if r.MatchString(host) {
						...
						logger.WithFields(logrus.Fields{
							"app":         app.ID,
							"NGINX_VHOST": host,
						}).Warn("invalid NGINX_VHOST or HAPROXY_0_VHOST label")
					}
				}
			} else if s, ok := app.Labels["subdomain"]; ok {
			...
			newapp.Env = app.Env
			newapp.NAME = strings.Replace(app.ID, "/", ".", -1)
			...
			newapp.NUM = strconv.Itoa(index)
			apps[strconv.Itoa(index)] = newapp
			index++
			//支持重复域名根据目录分发配置
			for _, host := range newapp.Hosts {
				if host != "" {
					servers[host] = append(servers[host], newapp)
				}
			}
